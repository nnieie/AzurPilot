...
class RewardShop(ShopUI):
    def run_frequent(self):
        self.shop_debug_start(label='frequent')
        try:
            self.ui_goto_shop()
            ...
            logger.exception('Shop frequent task failed')
            raise
        finally:
            self.shop_debug_stop()

    def run_once(self):
        self.shop_debug_start(label='once')
        try:
            # Munitions shops
            self.ui_goto_shop()
            self.device.click_record_clear()
            ...
            logger.exception('Shop once task failed')
            raise
        finally:
            self.shop_debug_stop()
